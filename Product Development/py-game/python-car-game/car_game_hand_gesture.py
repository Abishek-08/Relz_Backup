...
def gesture_detection():
    global gesture_direction, gesture_active
    ...
    while True:
        ...
        if results.multi_hand_landmarks:
            for hand_landmarks in results.multi_hand_landmarks:
                ...
                # Reset if hand comes back to center
                if 0.3 <= x <= 0.7:
                    gesture_active = False
        else:
            gesture_active = False  # Reset if no hand

        # Optional: show webcam window (comment for faster FPS)
...
while running:
    ...
    # Gesture movement with cooldown
    if gesture_direction and gesture_cooldown == 0:
        if gesture_direction == 'left' and current_lane_index > 0:
            ...
            print("Moved LEFT to lane", current_lane_index + 1)
            gesture_cooldown = 15

        elif gesture_direction == 'right' and current_lane_index < len(lanes) - 1:
            current_lane_index += 1
            player.rect.centerx = lanes[current_lane_index]
            print("Moved RIGHT to lane", current_lane_index + 1)
            gesture_cooldown = 15
