...
import {
  ResetLearnCreationAction,
} from "../../../redux/actions/admin_module_actions/LearningAssessmentCreationAction";
import { ToastContainer, toast } from "react-toastify";
import { PostLevelZeroAssessment, addLevelThreeLearningAssessment, submitModerateAssessment, submitQuickAssessment } from "../../../services/admin_module_services/LearningAssessmentService";
...
const AssessmentSchedule_Learning_Redux = () => {
  ...
  //Learning Assessment Creation - Redux
  const tabNumber = useSelector(
    (state) => state.learningCreationTabNumber.number
  );
  const L0Data = useSelector((state) => state.learningCreationTabNumber.L0Data);
  const L1Data = useSelector((state) => state.learningCreationTabNumber.L1Data);
  const L3Data = useSelector((state) => state.learningCreationTabNumber.L3Data);

  //L2 Data

  const l2topicId = useSelector((state) => state.levelTwoReducer.topicId);
  const l2subTopicIds = useSelector(
    (state) => state.levelTwoReducer.subTopicIds
  );
  const l2subTopic = useSelector((state) => state.levelTwoReducer.subTopic);
  const l2selectedSubTopic = useSelector(
    (state) => state.levelTwoReducer.selectedSubTopic
  );
  const l2levelTwoInputTotal = useSelector(
    (state) => state.levelTwoReducer.levelTwoInputTotal
  );
  const l2passPercentage = useSelector(
  ...
  const handleOrderSchedule = async () => {
    try {
      ...
      if (tabNumber === "1") {
        ...
        await PostLevelZeroAssessment(finalPayload);
      }
      ...
      if (tabNumber === "2") {
        ...
        await submitQuickAssessment(finalPayload);
      }

      if (tabNumber === "3") {
        const levelTwo = {
          topicId: l2topicId,
          subTopic: l2subTopicIds.map((id) => ({ subtopicId: id })),
          passMark: l2levelTwoInputTotal,
          assessment: {
            assessmentId: assessmentIdData,
          },
          numberOfQuestion: l2passPercentage,
        };

        await submitModerateAssessment(levelTwo);
      }
      ...
      if (tabNumber === "4") {
        ...
        await addLevelThreeLearningAssessment(finalPayload);
      }
...
export default AssessmentSchedule_Learning_Redux;
