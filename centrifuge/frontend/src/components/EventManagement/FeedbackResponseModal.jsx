...
import { ChevronDown, Search, BarChart3, PieChart as PieChartIcon, Activity, Mountain, Target, ArrowLeft, ChevronLeft, ChevronRight, Info } from 'lucide-react';



function FeedbackResponseModal({ feedbackData }) {
          ...
          <div className="absolute top-full left-0 right-0 mt-2 bg-white border border-gray-200 rounded-xl shadow-xl z-10 overflow-hidden">
            {chartOptions.map((option) => {
              ...
              return (
                ...
                >
                  <OptionIcon
                    className={`w-4 h-4 ${isSelected ? "text-[#274c77]" : "text-gray-400"
                      }`}
                  />
                  {option.label}
                  {isSelected && (
  ...
  const totalUsers = [...new Set(feedbackData.map((item) => item.feedbackUser.feedbackUserId))].length;
  console.log("totalUser", totalUsers);
  const allUsers = [...new Set(feedbackData.map((item) => item.feedbackUser.feedbackUserId))].map(userId => { const userResponse = feedbackData.find(item => item.feedbackUser.feedbackUserId === userId); return { id: userId, name: userResponse.feedbackUser.feedbackUserName, email: userResponse.feedbackUser.feedbackUserEmail, isVerified: userResponse.feedbackUser.isVerified }; });
  ...
  const QuestionDropdown = ({
    ...
    return (
      <div className="relative w-full mt-2">
        ...
        >
          <span
            className={`${!selectedQuestion ? "text-gray-500" : ""} hover:cursor-pointer`}
          >
            {selectedLabel}
          </span>
          <ChevronDown
            className={`w-4 h-4 text-[#274c77] transition-transform ${isOpen ? "rotate-180" : ""
              }`}
          />
        </button>
        {isOpen && (
  ...
  return (
    <div className=" h-full w-full flex flex-col overflow-hidden">
      ...
      <div className="flex p-4 items-center justify-between mb-1 flex-wrap gap-4">

        <div className="flex justify-start flex-grow">
          <div className="flex gap-1 p-1 bg-gray-100 rounded-lg">
            <button
              onClick={() => handleViewChange("overview")}
              className={`px-4 py-2 rounded-md hover:cursor-pointer font-medium transition-all duration-200 ${selectedView === "overview"
            ...
            <button
              ...
              className={`px-4 py-2 rounded-md hover:cursor-pointer font-medium transition-all duration-200 ${selectedView === "individual"
                ? "bg-[#274c77] text-white shadow-md"
                : "text-gray-700 hover:bg-gray-200"
                }`}
            >
            ...
            <button
              ...
              className={`px-4 py-2 rounded-md hover:cursor-pointer font-medium transition-all duration-200 ${selectedView === "users"
                ? "bg-[#274c77] text-white shadow-md"
                : "text-gray-700 hover:bg-gray-200"
                }`}
            >
          ...
          (selectedView === "individual" && selectedQuestion)) && (
            <div className="flex items-center gap-3">
              ...
              <ChartTypeDropdown
                ...
                isOpen={isChartDropdownOpen}
                setIsOpen={setIsChartDropdownOpen}
              />
            </div>
          )}
      </div>
      ...
      >
        ...
        {selectedView === "overview" && (
          <div className="px-2">
            ...
            <div className="h-64 w-full mb-4">{renderChart(overviewData)}</div>
              ...
              {overviewData.map((item) => (
                ...
                >
                  ...
                  <div className="text-xs opacity-75">
                    {(
                      (item.count / (totalUsers * uniqueQuestionsForDropdown.length)) *
                      100
                    ).toFixed(1)}
                    %
                  </div>
          ...
          <div className="px-2">
            ...
            <div className="mb-4">
              <QuestionDropdown
                ...
                isOpen={isQuestionDropdownOpen}
                setIsOpen={setIsQuestionDropdownOpen}
              />
            </div>
                  ...
                  <tbody className="divide-y divide-gray-200">
                    {paginatedUsers.map((user, userIndex) => {
                      ...
                      return (
                        <tr
                          key={user.id}
                          className={`hover:bg-gray-50 ${userIndex % 2 === 0 ? "bg-white" : "bg-gray-50/30"
                            }`}
                        >
                          <td className="sticky left-0 bg-inherit px-4 py-3 font-medium text-gray-900 border-r border-gray-200">
                            {user.name?.toLowerCase() === "anonymous" || user.email?.toLowerCase() === "anonymous"
                              ? "Anonymous"
                          ...
                          {uniqueQuestionsForDropdown.map((questionObj, qIndex) => {
                            const response = userResponses.find(
                              (r) => r.feedbackQuestion.feedbackQuestion === questionObj.question
                            );
                            const rating = response ? response.feedbackResponse : null;
                            return (
                              <td
                                key={qIndex}
                                ...
                                {rating ? (
                                  <div className="group relative inline-block">
                                    <div
                                      ...
                                      style={{ backgroundColor: emojiLabels[rating]?.color }}
                                    >
                                      {rating}
                                    </div>
                                    <div className="absolute bottom-full left-1/2 transform -translate-x-1/2 mb-2 px-2 py-1 bg-black text-white text-xs rounded opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none whitespace-nowrap z-10">
                                      <div className="text-lg">{emojiLabels[rating]?.emoji}</div>
                                      <div className="text-xs">{emojiLabels[rating]?.label}</div>
                                    </div>
                                  </div>
                                ) : (
                    ...
                    {filteredUsers.length === 0 && (
                      <tr>
                        <td colSpan={uniqueQuestionsForDropdown.length + 3} className="text-center py-6 text-gray-500">
                         Record not found!
                        </td>
                      </tr>
                    )}
                ...
                <div className="flex justify-center items-center gap-2 mt-4">
                  ...
                  {(() => {
                    ...
                    if (showEllipsisBefore) {
                      pages.push(
                        <span key="ellipsis-before" className="px-2 text-gray-500 font-medium">
                          ...
                        </span>
                      );
                    }
                    for (
                      ...
                      pages.push(
                        <button
                          ...
                          className={`px-3 py-1 rounded-md border text-sm font-medium ${currentPage === i
                            ? "bg-[#274c77] text-white"
                            : "text-gray-700 hover:bg-gray-100"
                            }`}
                        >
                    ...
                    if (showEllipsisAfter) {
                      pages.push(
                        <span key="ellipsis-after" className="px-2 text-gray-500 font-medium">
                          ...
                        </span>
                      );
                    }
